...
public:
    ...
    enum HTTP_CODE
    {
        ...
        UPLOAD_SUCCESS,
        UPLOAD_FAILURE,
        WEBSOCKET_UPGRADE    // WebSocket升级请求
    };
    enum LINE_STATUS
    {
        LINE_OK = 0,
...
public:
    ...
    void sendWebSocketPong(const string &payload);
    string recvWebSocketMessage();
...
private:
    ...
    bool m_upgrade_to_websocket = false;

     std::string m_room; // 加一个房间号
    void sendWebSocketMessage(const std::string& msg); // 补发消息函数声明
    void handle_websocket_message(const std::string& msg); // 信令处理声明
};
extern std::map<std::string, std::set<http_conn*>> g_webrtc_rooms; // 全局房间表
