...
void http_conn::close_conn(bool real_close)
{
    if (real_close && (m_sockfd != -1))
    {
        if (m_upload_in_progress && m_upload_fd != -1)
        {
            ...
            snprintf(path, FILENAME_LEN, "%s/upload/%s", doc_root, m_upload_filename.c_str());
            unlink(path);

            LOG_WARN("Upload aborted: %s", path);
        }
        printf("close %d\n", m_sockfd);
...
http_conn::HTTP_CODE http_conn::handle_webrtc_offer(const char *room_id)
{
    if (m_method == POST)
    {
        ...
        strncpy(session.offer, m_string, sizeof(session.offer) - 1);
        session.offer[sizeof(session.offer) - 1] = '\0';
    ...
    else if (m_method == GET)
    {
        ...
        if (it == webrtc_sessions.end() || it->second.offer[0] == '\0')
        {
            webrtc_lock.unlock();
            return NO_RESOURCE;
        }

        char *escaped_offer = json_escape(it->second.offer);
...
http_conn::HTTP_CODE http_conn::handle_webrtc_answer(const char *room_id)
{
    if (m_method == POST)
    {
        ...
        webrtc_lock.lock();
        WebRTCSession &session = webrtc_sessions[room_id];
        strncpy(session.answer, m_string, sizeof(session.answer) - 1);
        session.answer[sizeof(session.answer) - 1] = '\0';
        session.last_active = time(nullptr);
        webrtc_lock.unlock();
    ...
    else if (m_method == GET)
    {
        ...
        webrtc_lock.lock();
        auto it = webrtc_sessions.find(room_id);
        if (it == webrtc_sessions.end() || it->second.answer[0] == '\0')
        {
            webrtc_lock.unlock();
            return NO_RESOURCE;
        }

        char *escaped_answer = json_escape(it->second.answer);
        snprintf(json_buffer, sizeof(json_buffer),
                 "{\"type\":\"answer\", \"sdp\":\"%s\"}", escaped_answer);

        // 清理已获取的answer
        it->second.answer[0] = '\0';
        it->second.last_active = time(nullptr);
